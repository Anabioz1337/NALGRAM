...
def remote_build_darwin_containers(darwin_containers_path, darwin_containers_host, macos_version, bazel_cache_host, configuration, build_input_data_path):
    ...
    def handle_ssh_credentials(credentials):
        with DarwinContainers.ContainerSession(credentials=credentials) as session:
            ...
            if os.path.exists(artifacts_path + '/NALGRAM.ipa') or os.path.exists(artifacts_path + '/Telegram.ipa'):
                print('Artifacts have been stored at {}'.format(artifacts_path))
                sys.exit(0)
            else:
                print('Could not find NALGRAM.ipa or Telegram.ipa in {}'.format(artifacts_path))
                sys.exit(1)
    ...
    def handle_ssh_credentials(credentials):
        with DarwinContainers.ContainerSession(credentials=credentials) as session:
            ...
            guest_upload_sh = '''
                ...
                FASTLANE_PASSWORD="{password}" xcrun altool --upload-app --type ios --file "{ipa_filename}" --username "{username}" --password "@env:FASTLANE_PASSWORD"
            '''.format(username=username, password=password, ipa_filename=ipa_filename)
